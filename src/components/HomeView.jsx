import { FaThermometerHalf, FaWind, FaTint, FaMapMarkerAlt } from 'react-icons/fa';
...
export default function HomeView({ tambons, weatherStatus, lastUpdated, onRefresh }) {
  const now = new Date();
  const dateStr = `วัน${DAY_TH[now.getDay()]} ${now.getDate()} ${MONTH_TH[now.getMonth()]} ${now.getFullYear() + 543}`;

  if (!tambons || tambons.length === 0) {
    return (
      <div className="absolute inset-0 flex items-center justify-center pb-16" style={{ background: 'linear-gradient(180deg,#eff6ff 0%,#f8faff 100%)' }}>
  ...
  const tempColor   = getTemperatureColor(parseFloat(avgTemp));
  ...
  return (
    ...
    >
      <div className="max-w-md mx-auto px-4 pt-5 pb-4 space-y-3">
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-blue-600/70 text-xs mt-1">อ.เมืองขอนแก่น · {tambons.length} ตำบล</p>
            </div>
            <WeatherIllustration />
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="relative h-0">
              <div
                className="absolute -top-3.5 w-3 h-3 rounded-full bg-white border-2 border-blue-500 shadow"
        ...
        <div className="grid grid-cols-3 gap-3">
          {/* PM2.5 */}
          <div className="rounded-3xl p-3 bg-white col-span-1 flex flex-col items-center gap-1"
            ...
            {[...tambons].sort((a, b) => b.temperature - a.temperature).map(d => {
              ...
              return (
                <div key={d.id} className="rounded-2xl px-4 py-3 bg-white flex items-center gap-3"
                  ...
                  {/* Temp badge */}
                  <div className="w-10 h-10 rounded-2xl flex flex-col items-center justify-center flex-shrink-0"
